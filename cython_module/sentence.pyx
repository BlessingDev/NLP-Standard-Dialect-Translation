...
def batch_process_exa_output(list sentence_list, int batch_size):
    cdef str target

    for idx in range(batch_size):
        target = sentence_list[idx]

        target = target.split('\n')[-1]
        
        target = target[13:]

        sentence_list[idx] = target

def batch_process_llama_output(list sentence_list, int batch_size):
    for idx in range(batch_size):
        sentence_list[idx] = sentence_list[idx][0]["generated_text"][-1]["content"]
...
def batch_process_llama_templates(list sta_source_sentences, str prompt_template, int batch_size):
    cdef list messages
    cdef list sta_messages
    cdef str prompt
    ...
    for idx in range(batch_size):
        ...
        messages = [
            {"role": "system", 
            "content": "You are a multi-lingual chatbot who can perform translations."},
            {"role": "user", "content": prompt}
        ]
        sta_messages.append(messages)
